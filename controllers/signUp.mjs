...
async function sendVerificationMail(req, res, next) {
  ...
  try {
    ...
    await connection.execute('INSERT INTO tempToken (name, email, password, token) VALUES (?, ?, ?, ?)', 
      [req.body.name, req.body.email, hashedPassword, token]);
    let url = `http://107.178.98.186:3000/api/signUp/tokenValidation/${uriEncodedToken}`;
